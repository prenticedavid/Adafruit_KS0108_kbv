...
#include "Adafruit_ST7567_kbv.h"

#define ST7567_swap(a, b) (((a) ^= (b)), ((b) ^= (a)), ((a) ^= (b)))

#ifdef USE_HW_I2C
#include <Wire.h>

void Adafruit_ST7567_kbv::sendCmd(byte b)
{
...
void Adafruit_ST7567_kbv::sendCmd2(byte b1, byte b2)
{
            Wire.beginTransmission(i2caddr);
...
void Adafruit_ST7567_kbv::sendData(byte b)
{
            Wire.beginTransmission(i2caddr);
...
void Adafruit_ST7567_kbv::ST7567Init(void)
{
    Wire.begin();
    i2caddr = 0x3F;
...
Adafruit_ST7567_kbv::Adafruit_ST7567_kbv(int8_t LED)  : Adafruit_GFX(ST7567_WIDTH, ST7567_HEIGHT)
{
    led = LED;
}
#else
...
#define ST7567Setting     SPISettings(1000000, MSBFIRST, SPI_MODE0)
...
void Adafruit_ST7567_kbv::sendCmd(byte b)
{
    SPI_beginTransaction(ST7567Setting);
    digitalWrite(cs, LOW);
...
Adafruit_ST7567_kbv::Adafruit_ST7567_kbv(int8_t CS, int8_t DC, int8_t RST, int8_t LED)
                    : Adafruit_GFX(ST7567_WIDTH, ST7567_HEIGHT)
{
    cs = CS;
    dc = DC;
    rst = RST;
...
void Adafruit_ST7567_kbv::begin(void)
{
    ST7567Init();   //pinMode, reset, SPI, ...
    ...
    sendCmd(0x28|4);  //all power  control circuits on
    delay(50);
    sendCmd(0x28|6);  //all power  control circuits on
    delay(50);
    sendCmd(0x28|7);  //all power  control circuits on
    delay(50);
    sendCmd(0x23);  //v0 voltage resistor ratio
    sendCmd(0x81);sendCmd(54); //contrast
...
void Adafruit_ST7567_kbv::drawPixel(int16_t x, int16_t y, uint16_t color)
{
    ...
    switch (getRotation()) {
        ...
        uint8_t old = d;
    switch (color) {
        case ST7567_WHITE:   d |= mask; break;
        case ST7567_BLACK:   d &= ~mask; break;
        case ST7567_INVERSE: d ^= mask; break;
    }
...
void Adafruit_ST7567_kbv::fillRect(int16_t x0, int16_t y0, int16_t w, int16_t h, uint16_t color)
{
#if 1
    ...
    for (uint8_t y = y0 & ~7; y < 64 && h > 0; y += 8) {
        ...
        for (int x = x0; x < x0 + w; x++, p++) {
            ...
            if (color == ST7567_WHITE) newb |= mask;
            if (color == ST7567_INVERSE) newb |= oldb ^ mask;
            if (newb != oldb) {
                *p = newb;
...
void Adafruit_ST7567_kbv::clearDisplay(void)
{
    fillRect(0, 0, width(), height(), ST7567_BLACK);
}

void Adafruit_ST7567_kbv::invertDisplay(bool i)
{
    _xor = (i) ? 0xFF : 0x00;
    display();
}

void Adafruit_ST7567_kbv::display(void)
{
    //ST7567Blit((const uint8_t*)buffer, 0);  //use SRAM
    ...
    ST7567BlitRect((const uint8_t*)buffer, _left, _top, _rt - _left + 1, _bot - _top + 1, 0);  //use SRAM
    _left = 127;
    _rt = 0;
    _top = 63;
    _bot = 0;
...
void Adafruit_ST7567_kbv::backlight(bool on)
{
    if (led > 0) {
        pinMode(led, OUTPUT);
        digitalWrite(led, on);
...
void Adafruit_ST7567_kbv::ST7567BlitRect(const uint8_t *buf, uint8_t x0, uint8_t y0, uint8_t w, uint8_t h, uint8_t isflash)
{
    //x0 += 4;
